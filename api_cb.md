...
줄바꿈(`\n`)이 그대로 있으므로 `white-space: pre-wrap` 등으로 렌더링하세요.

값의 형태는 아래 3단 구성입니다:

```
[내가 할 일]
거주지 읍·면·동 주민센터나 근로복지공단에 방문해요. 신분증과 신청서를 준비해 서비스 신청을 해요.

[그다음 진행 과정]
신청 후에는 담당 시·군·구청이나 근로복지공단이 조사와 심사를 거쳐 지원 여부를
결정하고 서비스를 제공해요. …

[용어 설명]
- 주민센터: 거주지의 복지 관련 신청을 접수하는 가까운 행정기관이에요.
- 근로복지공단: 근로자의 산업재해와 복지 관련 업무를 담당하는 기관이에요.
```

- **856건 중 827건(97%)에 값이 있습니다.** 없는 29건은 `직접 지원(별도 신청사항 없음)`
  처럼 신청 자체를 받지 않는 제도라, 안내할 절차가 없어서 비어 있는 것이 정상입니다.
  이 29건은 `required_documents_ai`도 빈 배열입니다(정확히 같은 제도).
- 8번의 말풍선 A(`POST .../translate`)와는 **별개 필드**입니다. 말풍선 A는 대화
  State를 읽어 매번 새로 만드는 값이고, 이건 배치가 미리 만들어 둔 캐시된 값이라
  이 응답에 그냥 실려 옵니다. 말풍선 A는 신청 방법을 아예 다루지 않으므로 내용이
  겹치지 않습니다.
