...
# --- A-0. 검색 결과에 노출하지 않기로 한 제도 ---------------------------------------
# 자격은 맞을 수 있지만 이 서비스의 상담 맥락에서 쓸모가 없다고 **사람이 판단해**
# 뺀 목록이다. 아래 규칙들과 성격이 다르다 — 저쪽은 원문에서 근거를 읽어내지만
# 이건 근거가 코드 밖에 있다. 그래서 아주 좁게 유지하고, 넣을 때마다 이유를 적는다.
#
# WLF00000027 「장애인 운전교육 사업」 (2026-08-01)
#   중증장애인의 운전면허 실기·순회 교육이다. 지원대상이 "장애 유형(필수)
#   지체장애, 뇌병변장애, 청각장애"로 시작해서 장애 돌봄 상담의 어휘와 거의
#   전부 겹치고, 실측에서 지체장애 간병 상담 여러 건의 맞춤 상위에 반복해서
#   올라왔다. 질환 가산을 끊은 뒤에도 유사도만으로 다시 1위가 됐다.
#   돌봄·간병을 찾는 사용자에게 운전면허 교육을 맨 앞에 보여주면 목록 전체의
#   신뢰가 깎인다.
#
# 검색 결과에서만 뺀다. 상세 조회(db.fetch_institution)는 건드리지 않는다 —
# 저장해 둔 사용자가 열었을 때 404가 뜨면 안 된다.
...
EXCLUSIVE_GROUPS: Dict[str, Tuple[Tuple[str, ...], Tuple[str, ...]]] = {
    ...
    "환경오염피해": (("환경오염피해",), ("환경오염",)),
    "농어업인": (
        # '창업농'·'후계농'·'농촌출신'을 함께 본다. 실측(2026-08-01, 페르소나2):
        # 「청년창업농장학금 지원」이 배우자 간병 상담의 맞춤 구간에 올라왔다.
        # 이 제도의 농업 근거는 요약(serv_dgst)의 "농업 후계인력과 농업인 자녀"에만
        # 있는데, 이 판정은 제도명+지원대상만 본다. 지원대상은 "농림축산식품분야에
        # 취창업하고자 하는 대학생"이라 기존 어휘가 하나도 안 걸렸다.
        # 제도명의 '창업농'으로 잡는 것이 가장 확실하다.
        #
        # '농림축산식품'은 넣지 않는다 — 주관 부처 이름이라 「학교우유급식」처럼
        # 농업인 전용이 아닌 제도까지 끌려온다.
        ("농업인", "어업인", "영농", "농어업인", "농가", "창업농", "후계농", "농촌출신"),
        ("농사", "농업", "어업", "농가", "영농"),
    ),
}
...
DISEASE_BOOST_GROUPS: Dict[str, Tuple[Tuple[str, ...], Tuple[str, ...]]] = {
    ...
    # 왼쪽(사용자가 쓸 말)에 증상 확인 노드가 실제로 유도하는 표현을 함께 넣는다.
    # 그 질문은 "기억이나 판단이 예전 같지 않으신지"라고 묻는데(nodes._condition_ask),
    # 병명을 모르는 사용자는 "기억이 자꾸 흐려지세요"라고 답한다. 받아놓고 읽을
    # 어휘가 없으면 한 턴을 더 쓰고도 순위에 아무것도 싣지 못한다.
    "치매": (("치매", "인지증", "알츠하이머", "기억력", "기억이 흐", "기억을 잘 못",
              "기억이 잘 안", "깜빡깜빡", "못 알아보", "인지 저하", "인지기능"),
             ("치매",)),                                                     # 9
    ...
    # 오른쪽(제도 원문에서 찾을 말)에 **'지체장애'를 넣지 않는다.** 856건에서
    # '지체장애'가 나오는 4건은 전부 대상 장애유형을 나열한 자리다 —
    # 「장애인 운전교육 사업」의 "장애 유형(필수) 지체장애, 뇌병변장애, 청각장애",
    # 「장애인보조견전문훈련기관지원」, 「장애인인턴제」, 「희귀질환자 의료비 지원사업」.
    # 어느 것도 지체장애를 주 대상으로 하는 제도가 아닌데, 나열 한 줄에 걸리면
    # 질환일치가 되어 맞춤 최상단(cards._matched_order의 tier 0)을 차지한다.
    # 실측(2026-08-01, 페르소나2): 「장애인 운전교육 사업」이 중증도 불일치로
    # 0.7배 감점을 받고도 이 tier 때문에 맞춤 1위였다.
    #
    # '휠체어'(12) '보행'(4) '목발'(1)은 반대다. 걸리는 제도가 「장애인 휠체어 수리」
    # 「어르신 보행기 지원」「노인 성인용 보행기 지원」「의료급여 장애인보조기기」처럼
    # 거동이 불편한 사람에게 실제로 쓰이는 것들이라 그대로 둔다.
    "지체·보행": (("지체장애", "다리가 불편", "다리를 다치", "걷기 힘드", "걷지 못",
                  "걷는 게 힘", "걷는 것도 힘", "잘 못 걸", "걸음", "혼자 못 움직",
                  "움직이기 힘", "휠체어", "목발", "골절", "낙상", "넘어지"),
                  ("휠체어", "보행", "목발")),                                # 15
    "시각": (("시각장애", "눈이 안 보", "눈이 잘 안 보", "잘 안 보이", "앞이 안 보",
              "실명", "저시력"),
             ("시각장애",)),                                                 # 8
...
def apply(
    ...
    dropped: List[str] = []
    for row in rows:
        if row.get("serv_id") in ALWAYS_EXCLUDE:
            dropped.append("%s(노출 제외 목록)" % (row.get("serv_nm") or row.get("serv_id")))
            continue
