...
def converse_focus(state: CbState) -> Tuple[str, Dict[str, Any]]:
    ...
    **2는 턴을 새로 쓰지 않는다.** route_after_intent는 몸 상태를 보지 않으므로
    (거기 조건을 걸었다가 결과 화면이 한 턴 밀려서 되돌렸다 — 그쪽 주석 참고),
    이 질문은 어차피 되묻기로 정해진 턴 안에서 순서만 앞설 뿐이다. 실제로는
    소득 1단계와 자리를 맞바꾼다. 상태를 못 잡으면 질환 가산이 통째로 죽는 반면
    소득은 못 잡아도 '모르면 배제하지 않는다'로 넘어가므로 남는 거래다.
    ...
    if needs_condition_probe(state):
        # **소득 1단계 자리를 가져간다.** 이 질문이 순수하게 턴을 더 쓰면 결과
        # 화면까지 한 턴이 밀리고, 실제로 그것 때문에 제도 매칭으로 넘어가지
        # 못하는 일이 있었다(2026-08-01). 되묻는 턴의 총량은 그대로 두고
        # 무엇을 묻는지만 바꾼다 — needs_severity_probe와 같은 처리다.
        #
        # 잃는 것은 앵커 질문 한 번이고, 2·3단계(대략의 액수, 가구 안팎의 소득)는
        # 그대로 남는다. 상태를 못 잡으면 질환 가산이 통째로 죽는 반면 소득은
        # 못 잡아도 '모르면 배제하지 않는다'로 넘어가므로 남는 거래다.
