...
def route_after_intent(state: CbState) -> str:
    ...
    검색으로 넘어갈 때가 됐어도 장애 정도나 소득이 안 잡혔으면 대화 턴을 더 끼운다.

    **몸 상태 질문(needs_condition_probe)은 여기에 넣지 않는다.**
    한 번 넣었다가 되돌렸다(2026-08-01). 여기에 조건을 하나 더 걸면 결과까지
    턴이 하나 더 걸리고, 그만큼 결과 화면으로 넘어가는 시점이 밀린다. 실제로
    대화가 끝난 것 같은데 phase가 gathering에 머물러 제도 매칭으로 넘어가지
    못하는 일이 생겼다.

    몸 상태는 converse_focus 안에서만 순서를 갖는다. 즉 **턴을 새로 쓰는 것이
    아니라, 어차피 되묻기로 정해진 턴에서 무엇을 물을지만 바꾼다.**
    거기서 소득 1단계보다 앞에 서므로 실제로는 소득 질문 한 번과 자리를 맞바꾼다
    (converse_focus 주석 참고). 상태를 모르면 질환 가산이 통째로 무동작인 반면
