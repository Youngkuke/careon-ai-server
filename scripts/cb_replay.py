...
SCENARIOS: Dict[str, List[str]] = {
    # --- 시연용 -------------------------------------------------------------
    # 아래 둘은 회귀 확인이 아니라 **보여주기 위한** 대본이다. 각 턴이 어떤 기능을
    # 드러내는지 주석으로 달아 두었으니, 시연 중에 짚어가며 쓰면 된다.

    # 시연1 — 병명을 한 번도 말하지 않은 사용자에게 치매 제도를 찾아준다.
    #
    # 보여줄 것:
    #   1턴 대상(caree)을 나이보다 먼저 확인한다
    #   3턴 "어디가 어떻게 불편하신지"를 서비스 예시를 곁들여 묻는다
    #        (이분법 질문이 아니다)
    #   4턴 사용자는 '치매'라는 말을 **하지 않는다.** "기억이 흐려지신다"는
    #        설명만으로 시스템이 치매 제도군을 인식한다
    #   6턴 등급을 직접 묻지 않고 '받고 계신 지원의 이름'으로 우회한다
    #   결과 「재가급여」「시설급여」「특별현금급여(가족요양비)」가 질환일치로 상단.
    #        전부 사용자가 말한 적 없는 제도이고, 주제 태그도 신체건강이 아니라
    #        생활지원·보호·돌봄이라 태그 검색만으로는 나오지 않는 것들이다
    "시연1_치매를말하지않은치매": [
        "어머니가 많이 편찮으셔서 제가 계속 돌보고 있어요",
        "어머니를 위해서 찾고 있어요",
        "저는 27살이고 어머니는 일흔아홉이세요",
        "기억이 자꾸 흐려지시고 저를 못 알아보실 때가 있어요",
        "장기요양등급은 받으셨어요",
        "받고 계신 지원은 잘 모르겠어요",
        "저 혼자 벌고 어머니랑 둘이 살아요",
        "이제 찾아주세요",
    ...
    # 시연2 — 본인 이야기와 돌보는 분 이야기가 섞인 대화를 갈라낸다.
    #
    # 보여줄 것:
    #   3턴 "돌봄이랑 병원비" 다음에 곧장 검색하지 않고 상태를 캐묻는다
    #   4턴 낙상·거동 설명이 그대로 순위에 실린다 (질환일치:지체·보행)
    #   6턴 "저도 일자리가"라는 **본인** 발화를 본인 축으로 귀속시킨다
    #   결과 청년 일자리 제도는 남고, 등록장애인 전용 일자리 제도는 내려간다
    #        (축불일치 표시). 노인 돌봄 제도와 청년 제도가 한 화면에 함께 나온다
    "시연2_본인과돌봄대상을가른다": [
        "할아버지 관련해서 제도를 받고 싶어요",
        "저는 26살이고 할아버지는 81세세요",
        "돌봄이랑 병원비가 제일 부담돼요",
        "작년에 낙상하시고 나서 잘 못 걸으세요",
        "받고 계신 지원은 잘 모르겠어요",
        "저도 일자리가 빠듯해서 걱정이에요",
        "네 그게 다예요",
    ...
    # 시연3 — 본인이 지쳤다고 말해도, 찾는 것은 배우자 몫이다.
    #
    # 보여줄 것:
    #   1턴 "저도 너무 지쳐요"를 **본인 대상 서비스 요구로 오인하지 않는다.**
    #        같은 발화에 있는 "와이프한테 뭐가 더 있을지"를 보고 대상을 caree로 잡는다
    #   3턴 호칭을 지어내지 않는다 ('와이프분/아내분'. '어머님'이 아니다)
    #   5턴 "지체장애 3급"을 법정 경계로 옮긴다 → 심한 장애인
    #        (2019년 등급제 폐지. 1~3급이 '심한'이다)
    #   결과 「장애인활동 추가지원(구비추가사업)」에 '중증도 일치' 가산이 붙는다.
    #        등급을 잘못 읽으면 이 제도가 오히려 감점을 받는다 — 시연 전에
    #        이 표시를 꼭 확인할 것
    "시연3_지쳤다는말과찾는대상은다르다": [
        "와이프 돌본 지 3년째인데 저도 이제 너무 지쳐요. "
        "근데 와이프한테 뭐가 더 있을지 그것부터 알고 싶어요",
        "저는 47살이고 와이프는 45살이에요",
        "교통사고 후유증으로 지체장애 판정을 받았어요",
        "지체장애 3급이요",
        "소득은 얼마 안 돼요",
        "네 그게 다예요",
    ],
    ...
    # 첫마디 반복 회귀 (2026-08-01). 실제 로그에서 세 턴이 연달아
    # "간병이 많이 힘드셨겠어요"로 시작했다.
    #
    # 확인할 것:
    #   - 매 턴 첫마디가 달라지는가. 같은 위로가 두 번 나오면 실패다.
    #   - 장애 정도 앵커 질문("받고 계신 장애 관련 지원") 바로 다음 턴에
    #     "받고 계신 지원이 있으신가요?"가 또 나오지 않는가.
    #     (앵커 질문이 소득 1단계를 소진하므로 다음은 2단계 = 대략의 액수여야 한다.)
    "간병_첫마디반복": [
        "남편 간병 때문에 너무 힘들어요",
        "저는 58살이고 남편은 61살이에요",
        "교통사고 후유증으로 지체장애 판정을 받은 상태야",
        "지체장애 3급을 받았어",
        "소득은 얼마 안 돼요",
        "네 그게 다예요",
    ],
    # 페르소나2 — 본인의 소진을 말하지만 찾는 것은 배우자 몫이다.
    #
    # 확인할 것:
    #   - "저도 지쳐요"를 본인 대상 서비스 요구로 오인하지 않는가 (target_for=caree 유지)
    #   - 지친 감정 표현과 정보 요청 대상을 섞지 않는가
    #   - 「장애인활동지원 구비추가」류가 맞춤에 오르는가
    "배우자돌봄_소진": [
        "와이프 돌본 지 3년째인데 저도 이제 너무 지쳐요. "
        "근데 와이프한테 뭐가 더 있을지 그것부터 알고 싶어요",
        "저는 47살이고 와이프는 45살이에요",
